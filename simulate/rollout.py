...
param_path = os.environ.get('CONTROLLER_PARAM_PATH')
...
def execute_actions(rollout_traj, num_eval_envs, perturb=False):
    def make_evaluator(env):
        ...
        return acting.Evaluator(
            ...
            eval_encoder_fn=functools.partial(make_encoder, deterministic=skip_encoder),
            num_eval_envs=num_eval_envs, # Change me!!!!,
...
if __name__ == '__main__':
    rollout_traj = np.zeros([16, 120, 247])
    rollout_traj = add_scene_to_traj(rollout_traj, None, scene='vis')
    # print(rollout_taj.shape)
    a = execute_actions(rollout_traj)
    rollout_traj = remove_scene_from_traj(a)
    print(a.shape)