...
def add_scene_to_traj(traj, waypoint, scene='hit', bar_height=1.25):
    if scene=='hit':
        ...
        # bar_qp = brax.QP.zero(shape=(traj.shape[0], traj.shape[1], 1,))
        # bar_qp.pos[..., 0]= x_ed / 2
        # bar_qp.pos[..., 1]= y_ed / 2
        # bar_qp.pos[..., 2]= bar_height
        # combined_traj = serialize_qp(add_box(deserialize_qp(combined_traj), bar_qp))
        ...
        return combined_traj
    # box_qp = brax.QP.zero(shape=(rollout_traj.shape[0], rollout_traj.shape[1], num_box,))
    # for i in range(num_box):
    #     init_box(box_qp, i)
    # rollout_traj = serialize_qp(add_box(rollout_qp, box_qp))
