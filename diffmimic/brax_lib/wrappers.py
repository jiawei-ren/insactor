...
class EvalWrapper(wrappers.EvalWrapper):
  ...
  def reset_ref(self, rng: jp.ndarray, ref_traj: jp.ndarray, mask: jp.ndarray) -> brax_env.State:

    reset_state = self.env.reset_ref(rng, ref_traj, mask)
...
class AutoResetWrapper(wrappers.AutoResetWrapper):
  ...
  def reset_ref(self, rng: jp.ndarray, ref_traj: jp.ndarray, mask: jp.ndarray) -> brax_env.State:
    state = self.env.reset_ref(rng, ref_traj, mask)
...
class VmapWrapper(wrappers.VmapWrapper):
  ...
  def reset_ref(self, rng: jp.ndarray, ref_traj: jp.ndarray, mask: jp.ndarray) -> brax_env.State:
    return jp.vmap(self.env.reset_ref)(rng, ref_traj, mask)