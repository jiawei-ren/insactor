...
if __name__ == '__main__':
    for fps in ['30']:
        frame_rate = {}
        with open('/INPUT_DIR/KITML/mocap_framerate.txt') as f:
            lines = f.read().splitlines()
        for l in lines:
            frame_rate[l.split(' ')[0]] = float(l.split(' ')[1])
        file_list = sorted(glob.glob("/INPUT_DIR/KITML/*.npy"))
        file_list = file_list[args.partition*100:(args.partition+1)*100]
        for in_file in tqdm(file_list):
            ...
            with open('/OUTPUT_DIR/kitml_processed/{}.npy'.format(action), 'wb') as f:
                np.save(f, demo_traj)
