...
def get_ang_vel(rot, dt, target_dt, gt=None):
    ...
    if gt is not None:
        plt.plot(x, gt, 'x')
        # plt.plot(x_target, ang , 'x')
        plt.plot(x_target, ang_smoothed, '-')
        # print(ang_smoothed.shape, x_target.shape)
        # print(ang_smoothed[-10:, 0])
        plt.show()
...
if __name__ == '__main__':
    for fps in ['30']:
        frame_rate = {}
        with open('/INPUT_DIR/HumanML3D/mocap_framerate.txt') as f:     # we need the framerate to estimate velocity
            lines = f.read().splitlines()
        for l in lines:
            frame_rate[l.split(' ')[0]] = float(l.split(' ')[1])
        file_list = sorted(glob.glob("/INPUT_DIR/HumanML3D/humanml3d/*.npy"))
        total_len = len(file_list)
        seg_len = total_len // args.num + 1
        file_list = file_list[args.partition*seg_len:(args.partition+1)*seg_len]
        for in_file in tqdm(file_list):
            try:
                action = os.path.basename(in_file).split('.')[0]
                ase_motion = np.load(in_file)

                ase_poses = ase_motion[:, 3:]
                ...
                pelvis_trans = ase_motion[:, :3]
                pelvis_trans = pelvis_trans[:, [1,0,2]]
                pelvis_trans[:, 0] *= -1  #important!
                pelvis_trans[:, 1] *= -1  #important!

                dt = 1 / frame_rate[os.path.basename(in_file)]
                ...
                with open('/OUTPUT_DIR/humanml3d_processed/{}.npy'.format(action), 'wb') as f:
                    np.save(f, demo_traj)
            except:
                continue
