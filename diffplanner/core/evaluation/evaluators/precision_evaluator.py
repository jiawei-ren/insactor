...
class PrecisionEvaluator(BaseEvaluator):
    ...
    def single_evaluate(self, results, gt_flag):
        ...
        with torch.no_grad():
            ...
            gt_argsmax = np.argsort(gt_dist_mat, axis=1)
            top_k_mat = calculate_top_k(argsmax, top_k=self.top_k)
            gt_top_k_mat = calculate_top_k(gt_argsmax, top_k=self.top_k)
            top_k_count = top_k_mat.sum(axis=0)
            gt_top_k_count = gt_top_k_mat.sum(axis=0)
            all_size = word_emb.shape[0]
        if gt_flag == 0:
            return top_k_count, all_size
        else:
            return gt_top_k_count, all_size
    ...
    def parse_values(self, values, gt_values):
        ...
        for top_k in range(self.top_k):
            ...
            metrics['Ground Truth R_precision Top %d (mean)' % (top_k + 1)] = gt_values[0][top_k]
            metrics['R_precision Top %d (conf)' % (top_k + 1)] = values[1][top_k]
            metrics['Ground Truth R_precision Top %d (conf)' % (top_k + 1)] = gt_values[1][top_k]
        return metrics
