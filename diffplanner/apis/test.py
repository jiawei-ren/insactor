...
def single_gpu_test(model, data_loader, pre_seq=None,trans_req=None, waypoint=False):
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            pre_seq_i = None if pre_seq is None else pre_seq[i*32:(i+1)*32]
            trans_req_i = None if trans_req is None else trans_req[i]
            result = model(return_loss=False, waypoint=waypoint, inference_kwargs={'pre_seq': pre_seq_i, 'trans_req': trans_req_i}, **data)
