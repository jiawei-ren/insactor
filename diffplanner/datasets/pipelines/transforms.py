...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        if motion.shape[-1] < self.mean.shape[0]:
            # print('Warning: padding motion')
            motion = np.concatenate([motion, np.zeros_like(motion[..., :self.mean.shape[0]-motion.shape[-1]])], axis=-1)
        elif motion.shape[-1] > self.mean.shape[0]:
