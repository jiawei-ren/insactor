...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        """
        if trans_req is not None:
            trans_req = trans_req[:x.shape[0]]
            x_beg, y_beg, x_ed, y_ed = trans_req[..., 0, None, None], trans_req[..., 1, None, None], trans_req[..., 2, None, None], trans_req[..., 3, None, None]
        
            all_x = [3*i for i in range(18)]
            all_y = [3*i+1 for i in range(18)]
            total_frame = x.shape[1]
            frames = total_frame // 4

            x_trans = th.zeros_like(x)
            x_trans[:, :frames, all_x] += x_beg
            x_trans[:, :frames, all_y] += y_beg
            x_trans[:, -1*frames:, all_x] += x_ed
            x_trans[:, -1*frames:, all_y] += y_ed


            noise = th.randn_like(x_trans)
            x_t = self.q_sample(x_trans, t, noise=noise)
            x[:, :frames, all_x+all_y] = x_t[:, :frames, all_x+all_y]
            x[:, -1*frames:, all_x+all_y] = x_t[:, -1*frames:, all_x+all_y]
