...
class ACTOREncoder(BaseModule):
    ...
    def forward(self, motion, motion_mask=None, condition=None):
        motion = motion[:, :, :self.input_feats].float()
        B, T = motion.shape[:2]
        ...
        if self.output_var:
            motion_mask = torch.cat((torch.ones(B, 2).to(motion.device), motion_mask), dim=1)
        else:
            motion_mask = torch.cat((torch.ones(B, 1).to(motion.device), motion_mask), dim=1)
    ...
    # # Human
    # def forward(self, motion, motion_length=None, motion_mask=None, condition=None):
    #     motion = motion[:, :, :self.input_feats].float()
    #     B, T = motion.shape[:2]
    #     motion = motion.view(B, T, -1)
    #     feature = self.skelEmbedding(motion)
    ...
    #     # raise
    #     if self.output_var:
    #         motion_mask = torch.cat((torch.ones(B, 2).to(motion.device), motion_mask.squeeze(-1)), dim=1)
    #     else:
    #         motion_mask = torch.cat((torch.ones(B, 1).to(motion.device), motion_mask.squeeze(-1)), dim=1)
    #     feature = feature.permute(1, 0, 2).contiguous()
    #     feature = self.pos_encoder(feature)
    #     feature = self.seqTransEncoder(feature, src_key_padding_mask=motion_mask)
    #     if self.use_final_proj:
    ...
    def load_pretrained(self, ckpt_path):
        ...
        motion_ckpt = OrderedDict()
        for key in checkpoint['state_dict'].keys():
            if key.startswith('motion_encoder'):
                new_key = key[15:]
                motion_ckpt[new_key] = checkpoint['state_dict'][key]
        self.load_state_dict(motion_ckpt)
...
class ACTORDecoder(BaseModule):
    ...
    def forward(self, input, motion_mask=None, condition=None):
        ...
        feature = self.seqTransDecoder(tgt=query, memory=input, tgt_key_padding_mask=motion_mask)
